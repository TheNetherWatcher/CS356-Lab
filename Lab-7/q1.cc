...
int main(int argc, char *argv[]) {
    ...
    // Apply Error Model on Node1 → Node2
    Ptr<RateErrorModel> em = CreateObject<RateErrorModel>();
    em->SetAttribute("ErrorRate", DoubleValue(1e-7));
    em->SetAttribute("ErrorUnit", StringValue("ERROR_UNIT_BIT"));
    dev1_2.Get(1)->SetAttribute("ReceiveErrorModel", PointerValue(em));
    ...
    // Enable IP forwarding on Node1 and Node2
    for (uint32_t i = 1; i < nodes.GetN() - 1; ++i) {
        Ptr<Ipv4> ipv4 = nodes.Get(i)->GetObject<Ipv4>();
        ipv4->SetAttribute("IpForward", BooleanValue(true));
    }
    ...
    UdpClientHelper client1(if1_2.GetAddress(1), port1);
    ...
    UdpClientHelper client2(if2_3.GetAddress(1), port2);
    ...
    // Enable PCAP for each link
    p2p.EnablePcap("q1-node0-1", dev0_1.Get(0), true);
    p2p.EnablePcap("q1-node1-2", dev1_2.Get(0), true);
    p2p.EnablePcap("q1-node2-3", dev2_3.Get(0), true);
